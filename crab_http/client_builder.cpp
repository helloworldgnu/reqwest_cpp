...
{
    client_builder_destroy(handle_);
}

ClientBuilder::uptr ClientBuilder::Build()
{
    auto builder = new_client_builder();
    if (!builder)
    {
        return nullptr;
    }
...
{
    auto builder = client_builder_add_root_certificate(handle_, cert_path.c_str());
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_danger_accept_invalid_certs(handle_, accept_invalid_certs);
    if (builder)
    {
        handle_ = builder;
    }
...
ClientBuilder *ClientBuilder::default_headers(std::unique_ptr<HeaderMap> headerMap)
{
    auto builder = client_builder_default_headers(handle_, headerMap->Handle());
    if (builder)
    {
        handle_ = builder;
...
{
    auto builder = client_builder_http09_responses(handle_);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http1_allow_obsolete_multiline_headers_in_responses(handle_, val);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http1_only(handle_);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http1_title_case_headers(handle_);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http2_adaptive_window(handle_, enable);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http2_initial_connection_window_size(handle_, size);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http2_initial_stream_window_size(handle_, size);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http2_max_frame_size(handle_, size);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_http2_prior_knowledge(handle_);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_https_only(handle_, enable);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_local_address(handle_, local_address.c_str());
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_max_tls_version(handle_, version.c_str());
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_min_tls_version(handle_, version.c_str());
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_no_trust_dns(handle_);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_pool_idle_timeout(handle_, millisecond);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_pool_max_idle_per_host(handle_, max);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_proxy(handle_, proxy->Handle());
    if (builder)
    {
        handle_ = builder;
        proxy->ResetHandle();
...
{
    auto builder = client_builder_redirect(handle_, max);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_referer(handle_, enable);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_resolve(handle_, domain.c_str(), socket_addr.c_str());
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_resolve_to_addrs(handle_, domain.c_str(), &socket_addr_array[0], socket_addr_array.size());
    if (builder)
    {
        handle_ = builder;
    }
...
ClientBuilder *ClientBuilder::resolve_to_addrs(const std::string &domain, std::initializer_list<const char *> &socket_addr_array)
{
...
{
    auto builder = client_builder_tcp_keepalive(handle_, millisecond);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_tcp_nodelay(handle_, enable);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_timeout(handle_, millisecond);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_connect_timeout(handle_, millisecond);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_tls_built_in_root_certs(handle_, tls_built_in_root_certs);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_tls_sni(handle_, tls_sni);
    if (builder)
    {
        handle_ = builder;
    }
...
{
    auto builder = client_builder_user_agent(handle_, value.c_str());
    if (builder)
    {
        handle_ = builder;
    }
