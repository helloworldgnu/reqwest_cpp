...
class Response
{
  ...
  private:
    template <typename... Args> static std::unique_ptr<Response> Create(Args &&...args)
    {
        struct make_unique_helper : public Response
        {
            explicit make_unique_helper(Args &&...a) : Response(std::forward<Args>(a)...)
            {
            }
        };
  ...
  public:
    ...
    std::unique_ptr<ResponseBody> bodyText();
    ...
    std::unique_ptr<ResponseBody> bodyTextWithCharset(const std::string &default_encoding);

    /// Get the full response body as `Bytes`.
    /// The difference from copy_to is : This fun Consumption ownership
    std::unique_ptr<ResponseBody> bodyBytes();
    ...
    std::unique_ptr<ResponseBody> copy_to();
    ...
    std::unique_ptr<RString> remote_addr();

    /// Get the `StatusCode` of this `Response`.
    int32_t status();

    /// Get the final `Url` of this `Response`.
    std::unique_ptr<RString> url();
    ...
    ///_ => "unreachable"
    std::unique_ptr<RString> version();
