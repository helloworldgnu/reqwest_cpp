...
extern "C"
{
    ...
    const uint8_t *bytes_content(ByteBuffer *handle);

    uint64_t bytes_len(ByteBuffer *handle);
    ...
    /// trusted store.
    ClientBuilder *client_builder_add_root_certificate(ClientBuilder *client_builder, const char *cert_path);
    ...
    ClientBuilder *client_builder_connect_timeout(ClientBuilder *client_builder, const uint64_t *millisecond);
    ...
    /// as a last resort.
    ClientBuilder *client_builder_danger_accept_invalid_certs(ClientBuilder *client_builder, bool accept_invalid_certs);
    ...
    /// parsing.
    ClientBuilder *client_builder_http1_allow_obsolete_multiline_headers_in_responses(ClientBuilder *client_builder, bool val);
    ...
    ClientBuilder *client_builder_http2_initial_connection_window_size(ClientBuilder *client_builder, uint32_t *size);

    /// Sets the `SETTINGS_INITIAL_WINDOW_SIZE` option for HTTP2 stream-level flow control.
    ///
    /// Default is currently 65,535 but may change internally to optimize for common uses.
    ClientBuilder *client_builder_http2_initial_stream_window_size(ClientBuilder *client_builder, uint32_t *size);
    ...
    ClientBuilder *client_builder_local_address(ClientBuilder *client_builder, const char *local_address);
    ...
    ClientBuilder *client_builder_pool_idle_timeout(ClientBuilder *client_builder, const uint64_t *millisecond);
    ...
    ClientBuilder *client_builder_resolve(ClientBuilder *client_builder, const char *domain, const char *socket_addr);
    ...
    /// to the conventional port for the given scheme (e.g. 80 for http).
    ClientBuilder *client_builder_resolve_to_addrs(ClientBuilder *client_builder, const char *domain, const char *const *socket_addr_array, uintptr_t len);

    /// Set that all sockets have `SO_KEEPALIVE` set with the supplied duration.
    ///
    /// If `None`, the option will not be set.
    ClientBuilder *client_builder_tcp_keepalive(ClientBuilder *client_builder, const uint64_t *millisecond);
    ...
    /// This requires the optional `default-tls`, `native-tls`, or `rustls-tls(-...)`
    /// feature to be enabled.
    ClientBuilder *client_builder_tls_built_in_root_certs(ClientBuilder *client_builder, bool tls_built_in_root_certs);
    ...
    RequestBuilder *request_builder_basic_auth(RequestBuilder *request_builder, const char *username, const char *password);
    ...
    RequestBuilder *request_builder_body_bytes(RequestBuilder *request_builder, const uint8_t *bytes, uintptr_t size);
    ...
    /// Set the request body from file.
    RequestBuilder *request_builder_body_file_wide(RequestBuilder *request_builder, const wchar_t *file_path, uintptr_t length);

    /// Set the request body from file.
    RequestBuilder *request_builder_body_file_with_name(RequestBuilder *request_builder, const char *file_name, const char *file_path);

    /// Set the request body from file.
    RequestBuilder *request_builder_body_file_with_name_wide(RequestBuilder *request_builder, const char *file_name, const wchar_t *file_path, uintptr_t path_len);
    ...
    /// header.
    RequestBuilder *request_builder_form(RequestBuilder *request_builder, const Pair *pairs, uintptr_t len);

    /// Add a `Header` to this Request.
    RequestBuilder *request_builder_header(RequestBuilder *request_builder, const char *key, const char *value);
    ...
    RequestBuilder *request_builder_json(RequestBuilder *request_builder, const Pair *pairs, uintptr_t len);
    ...
    RequestBuilder *request_builder_query(RequestBuilder *request_builder, const Pair *querys, uintptr_t len);
    ...
    Response *request_builder_send(RequestBuilder *request_builder);
    ...
    void resp_err_clear(Response *handle);

    HttpErrorKind resp_err_kind(Response *handle);

    const uint8_t *resp_err_msg(Response *handle);

    uint64_t resp_err_msg_len(Response *handle);

    /// Get the full response body as `Bytes`.
    /// The difference from copy_to is : This fun Consumption ownership
    /// Don't forget free
    ByteBuffer *response_bytes(Response *handle);

    /// Get the content-length of the response, if it is known.
    uint64_t response_content_length(Response *handle);
    ...
    const ByteBuffer *response_copy_to(Response *handle);

    void response_destroy(Response *handle);

    /// Get the `Headers` of this `Response`.
    const HeaderMap *response_headers(Response *handle);

    int32_t response_read(Response *response, uint8_t *buf, uint32_t buf_len);

    /// Get the remote address used to get this `Response`.
    const char *response_remote_addr(Response *handle);

    /// Get the `StatusCode` of this `Response`.
    int32_t response_status(Response *handle);
    ...
    ByteBuffer *response_text(Response *handle);
    ...
    ByteBuffer *response_text_with_charset(Response *handle, const char *default_encoding);

    /// Get the final `Url` of this `Response`.
    const char *response_url(Response *handle);
    ...
    ///_ => "unreachable"
    const char *response_version(Response *handle);
